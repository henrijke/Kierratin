...
export class ApiservicesService {
  ...
  getAutocomplete(address) {
    this.http.get<Autocomplete>(this.googleAutocompleteAddress + 'input=' + address + '&language=fi&components=country:fi&key=' + this.googleApiKey).subscribe( (data) => {
         this.options = [];
...
*/
  ...
  getRecyclingPointsWithoutList() {
    this.http.get(this.kierratysInfoApiAddress + '?lat=' + Number(this.userLat) + '&lng=' + Number(this.userLng) + '&limit=4', {responseType: 'text'}).subscribe( response => {
    ...
    buildRecyclingPointsList() {
        ...
        for (const marker of this.xmlData) {
            ...
            for (const point of this.recyclingPoints) {
                if (point.place_id === marker.attributes.paikka_id.nodeValue) {
                    if (!point.type_id.includes(marker.attributes.laji_id.nodeValue)) {
        ...
        // katsotaan että onko recyclingPoints listan markereissa kaikki recyclingListan ID:t

        let ok = true;

        for (const id of this.recyclingList) {
            const value = this.isPresentInList(id);
            if (value === false) {
                ok = false;
                console.log(value);
                break;
    ...
    isPresentInList(value) {

      let ok = false;

        for (const point of this.recyclingPoints) {
            for (const id of point.type_id) {
    ...
    createNewRecyclingPoint(marker) {
        ...
        this.recyclingPoints.push(new RecyclingPoint(marker.attributes.nimi.nodeValue,
            Number(marker.attributes.lat.nodeValue),
            Number(marker.attributes.lng.nodeValue),
            list, marker.attributes.paikka_id.nodeValue,
